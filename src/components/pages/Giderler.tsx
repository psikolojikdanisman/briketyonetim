...
import { tl, fd, today, uid, GIDER_LABEL } from '@/lib/storage';
...
// Kullanıcının daha önce 'diger' olarak eklediği benzersiz isimler
function ozelKategoriler(giderler: Gider[]): string[] {
  const isimler = giderler
    .filter(g => g.kategori === 'diger' && g.kategoriIsim)
    .map(g => g.kategoriIsim!);
...
export default function GiderlerPage({ data, onSave, showToast }: GiderlerProps) {
  ...
  const [digerSecili, setDigerSecili] = useState(''); // daha önce eklenen özel kategori
  ...
  function kaydet() {
    ...
    if (kategori === 'diger') {
      // Önce seçili özel kategori, yoksa yeni girilen isim
    ...
    const yeni: Gider = {
      id: uid(), tarih, kategori, kategoriIsim, tutar: t,
  ...
  const kategoriBazli = [...SABIT_KATEGORILER, ...ozelKats.map(() => 'diger' as GiderKategori)]
    .reduce((acc, kat, i) => {
      const isim = i < SABIT_KATEGORILER.length
        ? GIDER_LABEL[kat]
        : ozelKats[i - SABIT_KATEGORILER.length];
      const toplam = buAyGiderler
        .filter(g => i < SABIT_KATEGORILER.length
          ? g.kategori === kat
          : g.kategori === 'diger' && g.kategoriIsim === isim)
        .reduce((s, g) => s + g.tutar, 0);
      if (!acc.find(x => x.isim === isim)) acc.push({ isim, toplam });
      return acc;
    }, [] as { isim: string; toplam: number }[]);

  // Sabit kategorilerin bu ay toplamı
  const buAyKatToplam = SABIT_KATEGORILER.map(kat => ({
    isim: GIDER_LABEL[kat],
    toplam: buAyGiderler.filter(g => g.kategori === kat).reduce((s, g) => s + g.tutar, 0),
  }));
  const buAyDigerToplam = buAyGiderler
    .filter(g => g.kategori === 'diger')
    .reduce((s, g) => s + g.tutar, 0);
    ...
    <div>
          ...
          <div className="panel-body">
            <div className="frow c2">
              ...
              <div>
                ...
                <input type="number" step="0.01" placeholder="0.00" value={tutar} onChange={e => setTutar(e.target.value)} />
              </div>
            </div>
                ...
                <select value={kategori} onChange={e => { setKategori(e.target.value as GiderKategori); setDigerIsim(''); setDigerSecili(''); }}>
                  <optgroup label="— Sabit —">
                    {SABIT_KATEGORILER.map(k => (
                      <option key={k} value={k}>{GIDER_LABEL[k]}</option>
                    ))}
                  </optgroup>
            ...
            {/* Diğer seçilince: daha önce eklenmiş olanlar + yeni ekleme */}
            {kategori === 'diger' && (
              <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                      {ozelKats.map(k => (
                        <button
                          key={k}
                          className={`btn btn-sm ${digerSecili === k ? 'btn-primary' : 'btn-secondary'}`}
                          onClick={() => { setDigerSecili(k); setDigerIsim(''); }}
                        >
                          {k}
                        </button>
                      ))}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="ör: Elektrik, Kira..."
                    value={digerIsim}
                    onChange={e => { setDigerIsim(e.target.value); setDigerSecili(''); }}
                  />
                </div>
              </div>
            )}
            ...
            <div className="frow">
              <div>
                ...
                <input type="text" placeholder="Detay not..." value={aciklama} onChange={e => setAciklama(e.target.value)} />
              </div>
            </div>
          ...
          <div className="panel-body">
            <div className="stat-grid" style={{ marginBottom: 16 }}>
              <div className="stat-card">
                <div className="stat-label">Bu Ay Toplam Gider</div>
                <div className="stat-value c-red" style={{ fontSize: 22 }}>{tl(buAyToplam)}</div>
              </div>
              <div className="stat-card">
                <div className="stat-label">Kayıt Sayısı</div>
                <div className="stat-value" style={{ fontSize: 22 }}>{buAyGiderler.length}</div>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {buAyKatToplam.map(k => (
                <div key={k.isim} style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  padding: '8px 12px', background: 'var(--surface)',
                  borderRadius: 'var(--radius)', border: '1px solid var(--border)',
                }}>
                  <span style={{ fontSize: 13, color: 'var(--text2)' }}>{k.isim}</span>
                  <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 13, color: k.toplam > 0 ? 'var(--danger)' : 'var(--text3)' }}>
                    {tl(k.toplam)}
                  </span>
                </div>
              ))}
              {buAyDigerToplam > 0 && (
                <div style={{
                  ...
                  <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 13, color: 'var(--danger)' }}>
                    {tl(buAyDigerToplam)}
                  </span>
                </div>
              )}
      ...
      <div className="panel">
        <div className="panel-header">
          <div className="panel-title">Gider Kayıtları</div>
        </div>
        ...
        <div style={{
          ...
          {(filtreBaslangic || filtreBitis || filtreKategori !== 'hepsi') && (
            <button className="btn btn-secondary btn-sm" onClick={() => { setFiltreKategori('hepsi'); setFiltreBaslangic(''); setFiltreBitis(''); }}>
              ✕ Temizle
            </button>
          )}
        ...
        <div className="panel-body-0">
          <table>
            <thead>
              <tr><th>Tarih</th><th>Kategori</th><th>Tutar</th><th>Açıklama</th><th></th></tr>
            </thead>
            <tbody>
              {filtrelenmis.length === 0 ? (
                ...
                <tr key={g.id}>
                  ...
                  <td>
                    <span className={`badge ${g.kategori === 'mazot' ? 'b-blue' : g.kategori === 'makine_bakim' ? 'b-yellow' : g.kategori === 'kamyon_bakim' ? 'b-green' : 'b-gray'}`}>
                      {giderEtiket(g)}
                    </span>
                  </td>
