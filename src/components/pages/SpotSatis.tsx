...
export default function SpotSatisPage({ data, onSave, showToast }: SpotSatisProps) {
  ...
  async function kaydet() {
    try {
      ...
      showToast('Spot satış kaydedilemedi', false);
    }
  }
  async function sil(id: number) {
    try {
      onSave({ ...data, spotSatislar: data.spotSatislar.filter(s => s.id !== id) });
      await deleteSpotSatis(id);
    } catch {
  ...
  async function odemeKaydet() {
    try {
      ...
      const newOpErrors = {
        ...
        const yeniOdeme = { id: uid(), musteriId: mid, tutar: t, tarih: opTarih || today(), aciklama: opAciklama };
        onSave({ ...data, spotOdemeler: [...data.spotOdemeler, yeniOdeme] });
        await saveSpotOdeme(yeniOdeme);
        setSonOdeme({ musteriId: mid, musteriIsim: musteriIsimById(mid), musteriTel: musteriTelById(mid), tutar: t, tarih: opTarih || today(), aciklama: opAciklama, no: makbuzNo, spotBorcOncesi: eskiBorc });
