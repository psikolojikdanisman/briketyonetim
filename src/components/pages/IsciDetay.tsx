...
import { tl, fd, buHafta } from '@/lib/storage';
...
interface FiltreToolbarProps {
  siralama: 'yeni' | 'eski';
  setSiralama: (v: 'yeni' | 'eski') => void;
  filtremod: 'hepsi' | 'gun' | 'aralik';
  setFiltremod: (v: 'hepsi' | 'gun' | 'aralik') => void;
  gunTarih: string;
  setGunTarih: (v: string) => void;
  baslangic: string;
...
function FiltreToolbar({
  siralama, setSiralama,
  filtremod, setFiltremod,
  gunTarih, setGunTarih,
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        {(['yeni', 'eski'] as const).map(s => (
          <button
            key={s}
            className={`btn btn-sm ${siralama === s ? 'btn-primary' : 'btn-secondary'}`}
      ...
      {/* Tümü */}
      <button
        className={`btn btn-sm ${filtremod === 'hepsi' ? 'btn-primary' : 'btn-secondary'}`}
        onClick={() => setFiltremod('hepsi')}
      >
        Tümü
      </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <button
          ...
          onClick={() => { setGunTarih(gunKaydir(gunTarih, -1)); setFiltremod('gun'); }}
          style={{ padding: '4px 10px', fontWeight: 700 }}
        >‹</button>
        <div
          style={{
            padding: '4px 12px',
            background: filtremod === 'gun' ? 'rgba(45,122,79,.12)' : 'var(--surface)',
            border: `1px solid ${filtremod === 'gun' ? 'var(--accent)' : 'var(--border)'}`,
            borderRadius: 'var(--radius)',
            fontSize: 13,
            fontFamily: 'IBM Plex Mono, monospace',
            minWidth: 100,
            textAlign: 'center' as const,
            color: gunTarih === today() ? 'var(--accent)' : 'var(--text)',
            fontWeight: gunTarih === today() ? 700 : 400,
            cursor: 'pointer',
          }}
          onClick={() => { setGunTarih(today()); setFiltremod('gun'); }}
        >
          {filtremod === 'gun' ? (gunTarih === today() ? 'Bugün' : fd(gunTarih)) : 'Bugün'}
        </div>
        <button
          className="btn btn-sm btn-secondary"
          onClick={() => { setGunTarih(gunKaydir(gunTarih, +1)); setFiltremod('gun'); }}
      ...
      {/* Aralık */}
      <button
        className={`btn btn-sm ${filtremod === 'aralik' ? 'btn-primary' : 'btn-secondary'}`}
        onClick={() => setFiltremod(filtremod === 'aralik' ? 'hepsi' : 'aralik')}
      >
        Aralık
      </button>
      {filtremod === 'aralik' && (
...
export default function IsciDetay({ data, onSave, showToast, isciId, onGeri }: IsciDetayProps) {
  ...
  const [sonOdeme, setSonOdeme] = useState<{
    isciIsim: string; tutar: number; tarih: string; aciklama: string; no: string;
  ...
  const [ureGunTarih, setUreGunTarih] = useState(today());
  ...
  const [yukGunTarih, setYukGunTarih] = useState(today());
  ...
  const [odeGunTarih, setOdeGunTarih] = useState(today());
  ...
  const to = useMemo(() =>
    data.avanslar.filter(a => a.isciId === isciId).reduce((s, a) => s + a.tutar, 0),
    [data.avanslar, isciId]
  );
  ...
  const ho = useMemo(() =>
    data.avanslar.filter(a => a.isciId === isciId && a.tarih >= bas).reduce((s, a) => s + a.tutar, 0),
    [data.avanslar, isciId, bas]
  );
  ...
  const filtreliUretimler = useMemo(() => {
    ...
    else if (ureFiltremod === 'aralik') {
      if (ureBaslangic) list = list.filter(u => u.tarih >= ureBaslangic);
      if (ureBitis)     list = list.filter(u => u.tarih <= ureBitis);
    }
    return [...list].sort((a, b) => ureSiralama === 'yeni'
  ...
  const filtreliYuklemeler = useMemo(() => {
    ...
    else if (yukFiltremod === 'aralik') {
      if (yukBaslangic) list = list.filter(y => y.tarih >= yukBaslangic);
      if (yukBitis)     list = list.filter(y => y.tarih <= yukBitis);
    }
    return [...list].sort((a, b) => yukSiralama === 'yeni'
      ? b.tarih.localeCompare(a.tarih)
      : a.tarih.localeCompare(b.tarih)
    );
  ...
  const filtreliOdemeler = useMemo(() => {
    ...
    else if (odeFiltremod === 'aralik') {
      if (odeBaslangic) list = list.filter(a => a.tarih >= odeBaslangic);
      if (odeBitis)     list = list.filter(a => a.tarih <= odeBitis);
    }
    return [...list].sort((a, b) => odeSiralama === 'yeni'
      ? b.tarih.localeCompare(a.tarih)
      : a.tarih.localeCompare(b.tarih)
    );
  ...
  function odemeKaydet() {
    ...
    onSave({
      ...data,
      avanslar: [...data.avanslar, { id: Date.now() + Math.floor(Math.random() * 9999), isciId, tutar, tarih: avTarih, aciklama: avAciklama }],
    });
    setSonOdeme({
      isciIsim: isci.isim, tutar, tarih: avTarih, aciklama: avAciklama, no: makbuzNo,
      haftaKazanc: hk.top, haftaOdenenOncesi: ho,
      tumKazanc: tk.top, tumOdenenOncesi: to,
    });
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        <button
          ...
          onClick={onGeri}
          style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'IBM Plex Mono, monospace', fontSize: 12 }}
        >
          ← Geri
        </button>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'IBM Plex Mono, monospace', letterSpacing: 1 }}>
            İŞÇİ PROFİLİ
          </div>
        </div>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        ...
        <div className="panel-body">
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginTop: 4 }}>
            ...
            {sonOdeme && (
              <button className="btn btn-secondary" onClick={makbuzAc} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>📄 Makbuz PDF</button>
            )}
          </div>
          {sonOdeme && (
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        ...
        <FiltreToolbar
          siralama={ureSiralama} setSiralama={setUreSiralama}
          filtremod={ureFiltremod} setFiltremod={setUreFiltremod}
          gunTarih={ureGunTarih} setGunTarih={setUreGunTarih}
          baslangic={ureBaslangic} setBaslangic={setUreBaslangic}
          bitis={ureBitis} setBitis={setUreBitis}
          kayitSayisi={filtreliUretimler.length}
        />
        <div className="panel-body-0">
          <table>
            <thead>
              <tr><th>Tarih</th><th>Çeşit</th><th>Toplam Üretim</th><th>Kişi Başı Ücret</th><th>Toplam Havuz</th></tr>
            </thead>
            <tbody>
              {filtreliUretimler.length === 0
                ? <tr><td colSpan={5} className="empty">Bu filtre için üretim kaydı yok</td></tr>
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        ...
        <FiltreToolbar
          siralama={yukSiralama} setSiralama={setYukSiralama}
          filtremod={yukFiltremod} setFiltremod={setYukFiltremod}
          gunTarih={yukGunTarih} setGunTarih={setYukGunTarih}
          baslangic={yukBaslangic} setBaslangic={setYukBaslangic}
          bitis={yukBitis} setBitis={setYukBitis}
          kayitSayisi={filtreliYuklemeler.length}
        />
        <div className="panel-body-0">
          <table>
            <thead>
              <tr><th>Tarih</th><th>İşlem</th><th>Miktar</th><th>Kişi Başı Ücret</th><th>Not</th></tr>
            </thead>
            <tbody>
              {filtreliYuklemeler.length === 0
                ? <tr><td colSpan={5} className="empty">Bu filtre için kayıt yok</td></tr>
      ...
      <div className="panel">
        ...
        <FiltreToolbar
          siralama={odeSiralama} setSiralama={setOdeSiralama}
          filtremod={odeFiltremod} setFiltremod={setOdeFiltremod}
          gunTarih={odeGunTarih} setGunTarih={setOdeGunTarih}
          baslangic={odeBaslangic} setBaslangic={setOdeBaslangic}
          bitis={odeBitis} setBitis={setOdeBitis}
          kayitSayisi={filtreliOdemeler.length}
        />
        <div className="panel-body-0">
          <table>
            <thead>
            ...
            <tbody>
              {filtreliOdemeler.length === 0
                ? <tr><td colSpan={4} className="empty">Bu filtre için ödeme kaydı yok</td></tr>
