...
import { tl, fd, today, uid, musteriAlacak, SIP_CESIT_LABEL, saveMusteri, deleteMusteri, saveMusteriOdeme, deleteMusteriOdeme, saveGecmisBorc, deleteGecmisBorc, deleteSiparis, deleteTeslimat, deleteSpotSatis, deleteSpotOdeme } from '@/lib/storage';
...
export default function MusterilerPage({ data, onSave, showToast }: MusterilerProps) {
  ...
  async function musteriEkle() {
    try {
      ...
      const yeni = { id: uid(), isim: isim.trim(), tel: tel.trim(), bolge: koyObj?.bolge || '', koy: koySecili, adres: adres.trim() };
      onSave({ ...data, musteriler: [...data.musteriler, yeni] });
      await saveMusteri(yeni);
      setIsim(''); setTel(''); setKoyAra(''); setKoySecili(''); setAdres('');
      showToast('Müşteri eklendi');
    } catch {
      showToast('Müşteri eklenemedi', false);
    }
  }
  ...
  async function musteriSilOnayla(id: number) {
    try {
      ...
      onSave({
        ...
        await Promise.all(data.siparisler.filter(s => s.musteriId === id).map(s => deleteSiparis(s.id)));
        await Promise.all(data.teslimatlar.filter(t => t.musteriId === id).map(t => deleteTeslimat(t.id)));
        await Promise.all(data.musteriOdemeler.filter(o => o.musteriId === id).map(o => deleteMusteriOdeme(o.id)));
        await Promise.all(data.spotSatislar.filter(s => s.musteriId === id).map(s => deleteSpotSatis(s.id)));
        await Promise.all(data.spotOdemeler.filter(o => o.musteriId === id).map(o => deleteSpotOdeme(o.id)));
        await Promise.all((data.gecmisBorclar || []).filter(g => g.musteriId === id).map(g => deleteGecmisBorc(g.id)));
        setSilOnay(null);
        showToast('Müşteri silindi');
    } catch {
      showToast('Müşteri silinemedi', false);
    }
  }
  async function odemeKaydet() {
    try {
      ...
      const yeniOdeme = { id: uid(), musteriId: mid, tutar, tarih: opTarih || today(), aciklama: opAciklama };
      onSave({ ...data, musteriOdemeler: [...data.musteriOdemeler, yeniOdeme] });
      await saveMusteriOdeme(yeniOdeme);
      setSonOdeme({ musteriId: mid, musteriIsim: musteri?.isim || '?', musteriTel: musteri?.tel || '', tutar, tarih: opTarih || today(), aciklama: opAciklama, no: makbuzNo, toplamBorc: eskiBorc });
