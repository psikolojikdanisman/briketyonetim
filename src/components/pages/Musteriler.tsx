...
import type { AppData, Teslimat } from '@/types';
import { tl, fd, today, uid, SIP_CESIT_LABEL } from '@/lib/storage';
...
export default function MusterilerPage({ data, onSave, showToast }: MusterilerProps) {
  ...
  function handleOpMusteriSec(val: string) {
    ...
    setSonOdeme(null);
    if (val) {
      const kalan = mbHesapla(parseInt(val));
  ...
  function gecmisBorcKaydet() {
    ...
    if (gbSeferSayisi) notParcalari.push(`Sefer sayısı: ${gbSeferSayisi}`);
    if (gbNot)         notParcalari.push(gbNot);
    const notMetni = notParcalari.length ? notParcalari.join(' | ') : 'Geçmiş borç girişi';
    const cesit = gbBriket20 ? '20lik' : gbBriket15 ? '15lik' : gbBriket10 ? '10luk' : gbCimento ? 'cimento' : gbKum ? 'kum' : 'gecmis';
    const yeniTeslimat: Teslimat = {
      id: uid(), siparisId: 0, musteriId: mid,
      cesit, bolge: '', koy: gbKoy, adres: notMetni,
      birimFiyat: 0, adet: 1, tutar: borc, tahsil: 0,
      odemeDurumu: 'veresiye', tarih: gbTarih || today(), birim: 'adet',
    };
    onSave({ ...data, teslimatlar: [...data.teslimatlar, yeniTeslimat] });
    ...
    <div>
      ...
      <div className="two-col">
          ...
          <div className="panel-body">
            ...
            <div className="frow" style={{ position: 'relative' }}>
              <div>
                ...
                <input type="text" placeholder="Arayın veya yazın..." value={koyAra}
            ...
            <div className="frow"><div><label>Adres / Not</label><input type="text" placeholder="Detaylı adres..." value={adres} onChange={e => setAdres(e.target.value)} /></div></div>
        ...
        <div className="panel">
          ...
          <div className="panel-body">
            ...
            {opMusteri && (() => {
              const kalan = mbHesapla(parseInt(opMusteri));
              return kalan > 0 ? (
            ...
            <div className="frow"><div><label>Açıklama</label><input type="text" placeholder="ör: kasım ödemesi" value={opAciklama} onChange={e => setOpAciklama(e.target.value)} /></div></div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn btn-success" onClick={odemeKaydet}>✓ Ödeme Al</button>
              {sonOdeme && (
                <button className="btn btn-secondary" onClick={musteriMakbuzAc} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  📄 Makbuz PDF
                </button>
              )}
            </div>
      ...
      <div className="panel">
        ...
        <div className="panel-body-0">
          <table>
            <thead><tr><th>Ad Soyad</th><th>Tel</th><th>Köy / Bölge</th><th>Alacak</th><th>Son İşlem</th><th></th><th></th></tr></thead>
            <tbody>
            ...
            <>
              <div className="stat-grid" style={{ marginBottom: 16 }}>
                <div className="stat-card">
                  ...
                  <div className="stat-value" style={{ fontSize: 22 }}>{detayTes.filter(t => t.cesit !== 'gecmis').reduce((s, t) => s + t.adet, 0).toLocaleString('tr-TR')}</div>
                  <div className="stat-sub">adet</div>
                </div>
                <div className="stat-card"><div className="stat-label">Toplam Tutar</div><div className="stat-value c-accent" style={{ fontSize: 20 }}>{tl(topTutar)}</div></div>
                <div className="stat-card"><div className="stat-label">Tahsil Edilen</div><div className="stat-value c-green" style={{ fontSize: 20 }}>{tl(topTahsil)}</div></div>
                <div className="stat-card"><div className="stat-label">Kalan Borç</div><div className={`stat-value ${detayBorc > 0 ? 'c-red' : 'c-green'}`} style={{ fontSize: 20 }}>{tl(detayBorc)}</div></div>
              </div>
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Tarih</th><th>Yer</th><th>Çeşit</th><th>Miktar</th><th>Birim Fiyat</th><th>Tutar</th><th>Tahsil</th><th>Kalan</th></tr></thead>
                  <tbody>
                    {[...detayTes].reverse().map(t => {
                      const k = t.tutar - t.tahsil;
                      const gecmis = t.cesit === 'gecmis';
                      const cesitLabel = gecmis ? 'Geçmiş' : (SIP_CESIT_LABEL?.[t.cesit] ?? t.cesit);
                      return (
                        <tr key={t.id} style={gecmis ? { background: 'rgba(255,200,0,.04)' } : undefined}>
                          <td>{fd(t.tarih)}</td>
                          <td style={{ maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.koy || t.bolge || '—'}</td>
                          <td><span className={`badge ${gecmis ? 'b-gray' : 'b-yellow'}`}>{cesitLabel}</span></td>
                          <td className="td-mono">{gecmis ? <span style={{ fontSize: 11, color: 'var(--text3)' }} title={t.adres}>{t.adres && t.adres !== 'Geçmiş borç girişi' ? t.adres.slice(0, 30) + (t.adres.length > 30 ? '…' : '') : '—'}</span> : t.adet.toLocaleString('tr-TR')}</td>
                          <td className="td-mono">{gecmis ? '—' : `${(t.birimFiyat || 0).toFixed(2)} TL`}</td>
