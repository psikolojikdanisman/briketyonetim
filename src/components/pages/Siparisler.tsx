...
export default function SiparislerPage({ data, onSave, showToast }: SiparislerProps) {
  ...
  async function siparisKaydet() {
    try {
      ...
      const newErrors = {
        ...
        await Promise.all(yeniSiparisler.map(s => saveSiparis(s)));
      ...
      showToast('Sipariş kaydedilemedi', false);
    }
  }
  ...
  async function siparisSilOnayla() {
    try {
      if (silinecekId === null) return;
      onSave({ ...data, siparisler: data.siparisler.filter(s => s.id !== silinecekId) });
      await deleteSiparis(silinecekId);
      setSilinecekId(null); setSilinecekMesaj('');
      showToast('Sipariş silindi');
    } catch {
      showToast('Sipariş silinemedi', false);
    }
  }
  async function siparisDuzenleKaydet(guncellendi: Siparis) {
    try {
      onSave({ ...data, siparisler: data.siparisler.map(s => s.id === guncellendi.id ? guncellendi : s) });
      await saveSiparis(guncellendi);
      setDuzenlenecek(null);
      showToast('Sipariş güncellendi ✓');
    } catch {
      showToast('Sipariş güncellenemedi', false);
    }
  }
  async function hizliMusteriEkle(m: Musteri, yeniKoy?: { isim: string; bolge: 'yakin' | 'uzak' }) {
    try {
      const yeniKoyObj = yeniKoy ? { id: uid(), isim: yeniKoy.isim, bolge: yeniKoy.bolge } : null;
      const yeniKoyler = yeniKoyObj ? [...data.koyler, yeniKoyObj] : data.koyler;
      const yeniData = { ...data, musteriler: [...data.musteriler, m], koyler: yeniKoyler };
      onSave(yeniData);
      await saveMusteri(m);
      if (yeniKoyObj) await saveKoy(yeniKoyObj);
  ...
  async function teslimatKaydet() {
    try {
      const newErrors = {
        ...
        await Promise.all(yeniTeslimatlar.map(t => saveTeslimat(t)));
        await Promise.all(yeniSiparisler.filter(s => seciliSiparisler.find(x => x.siparisId === s.id)).map(s => saveSiparis(s)));
      ...
      showToast('Teslimat kaydedilemedi', false);
    }
  }
