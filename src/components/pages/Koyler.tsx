...
import { uid, saveKoy, deleteKoy } from '@/lib/storage';
...
export default function KoylerPage({ data, onSave, showToast }: KoylerProps) {
  ...
  function teslimatSayisi(koyIsim: string) {
    return data.teslimatlar.filter(t => t.koy === koyIsim).length + data.spotSatislar.filter(s => s.koy === koyIsim).length;
  }
    ...
    <div>
        ...
        <div className="modal-overlay open">
          <div className="modal" style={{ maxWidth: 400 }}>
            ...
            <div style={{ background: 'rgba(184,60,43,0.06)', border: '1px solid rgba(184,60,43,0.18)', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {silOnay.teslimatSayisi > 0 && <div style={{ fontSize: 13, color: 'var(--text2)', fontFamily: 'JetBrains Mono, monospace' }}>🚛 Teslimat: <strong style={{ color: 'var(--red)' }}>{silOnay.teslimatSayisi} kayıt</strong></div>}
              {silOnay.spotSayisi > 0    && <div style={{ fontSize: 13, color: 'var(--text2)', fontFamily: 'JetBrains Mono, monospace' }}>⚡ Spot satış: <strong style={{ color: 'var(--red)' }}>{silOnay.spotSayisi} kayıt</strong></div>}
            </div>
            <p style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 20 }}>Köy silinir, ilişkili kayıtlar <strong>silinmez</strong> — sadece köy adı referanssız kalır.</p>
      ...
      <div className="two-col">
        <div className="panel">
          ...
          <div className="panel-body">
            ...
            <div className="frow">
              <div>
                <label>{bolge === 'merkez' ? 'Mahalle Adı' : 'Köy Adı'}{bolge === 'merkez' && <span style={{ color: 'var(--text3)', fontWeight: 400 }}> (isteğe bağlı)</span>}</label>
                <input type="text" placeholder={bolge === 'merkez' ? 'ör: Çarşı Mahallesi' : 'Köy adını girin'} value={isim} onChange={e => setIsim(e.target.value)} />
              </div>
            </div>
            <div className="frow">
              <div><label>Kısa Not (isteğe bağlı)</label><input type="text" placeholder="ör: Akçadağ yolu 12 km" value={notVal} onChange={e => setNotVal(e.target.value)} /></div>
            </div>
      ...
      <div className="panel">
        ...
        <div className="panel-body-0">
          <table>
            <thead><tr><th>Köy / Yer Adı</th><th>Bölge</th><th>Not</th><th>Toplam Teslimat</th><th>Toplam Adet</th><th></th></tr></thead>
            <tbody>
              {data.koyler.length === 0 ? (
                ...
                <tr key={k.id}>
                  ...
                  <td><span className={`badge ${k.bolge === 'merkez' ? 'b-blue' : k.bolge === 'yakin' ? 'b-yellow' : 'b-gray'}`}>{BOLGE_ADI[k.bolge]}</span></td>
